...
prop_uri_gen(N) ->
    in_parallel(
        ?FORALL(
            {_URIMap, URIBin, Fault},
            fault_rate(N, 100, uri_pair()),
            begin
                Normalize =
                ...
                collect(
                    with_title(path_types),
                    if
                        Normalize /= uri ->
                            invalid_uri;
                        true ->
                            {dot_segment, string:find(URIBin, "..") /= nomatch}
                    end,
                    collect(
                        with_title(fault_injection),
                        Fault,
                        check_distribution(
                            positive,
                            9 * (100 - N) / 1000,
                ...
                case riak_api_web_acceptor:split_path(URIBin) of
                    {ok, {Path, DecodedPath, Params}} ->
                        ...
                        collect(
                            ...
                            ?WHENFAIL(
                                eqc:format(
                                    "BinURI: ~p, Path: ~p, StartWithSlash: ~p, DecodedPath: ~p, Params: ~p\n",
                                    [
                                        URIBin,
                                        Path,
                                        StartWithSlash,
        ...
        try uri_string:recompose(recompose_query(URIMap)) of
            ...
            URI ->
                try uri_string:normalize(URI) of
                    ...
                    _:_ ->
                        {URIMap, unicode:characters_to_nfc_binary(URI),
                            non_normalizable}
                end
        catch
            _:_ ->
...
uri_map() ->
    ?LET(
        {Path, Params},
        {less_faulty(10, list(valid_path_element())), list(query_param())},
        maps:without(
            [query || Params == []],
            #{
                path => filename:join(["/" | Path]),
                scheme => less_faulty(
                    10, fault("http", oneof(["http", "https"]))
                ),
                host => fault(
                    "2001::bd:1",
                    oneof(["example.com", "localhost", "127.0.0.1"])
                ),
...
valid_path_element() ->
    %% percent is valid, but only to encode
    frequency([
        {5, list(elements(valid_chars(path) -- [$%]))},
        {1,
            fault(
                oneof([[0], " ", ">", unicode(), "%x"]),
                oneof([".", ".."])
